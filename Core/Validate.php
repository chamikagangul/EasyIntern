        ...
        if($rule == 'required' && empty($value)){
          ...
          switch($rule){
            ...
            case 'unique':
              ...
              if($check->count()){
                $this->addError(["{$display} is already exists please use another {$display}",$item]);
              }
              break;
            case 'unique_update':
             ...
             if($query->count()){
               $this->addError(["{$display} is already exists. please choose another",$item]);
             }
             break;
            case "is_numeric":
             if(is_numeric($value)){
            ...
            case 'validEmail':
              if(!filter_var($value,FILTER_VALIDATE_EMAIL)){
                $this->addError(["{$display} must be a email address.",$item]);
              }
              break;
          }
        }
  ...
  public function displayErrors(){
    ...
    foreach ($this->_errors as $error) {
      if(is_array($error)){
        ...
        $html.="<script>jQuery('document').ready(function(){jQuery('#".$error[1]."').addClass('border-warning');});</script>";
      }else{
